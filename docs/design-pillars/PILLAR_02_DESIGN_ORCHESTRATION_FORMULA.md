...
**Context:** The first execution attempt against this cluster's canonical PR Plan (Grok Build, PLAN_ID `d992f697`) landed real, verified work for Pillars 1 and 5 (9 feature commits + 1 review-fix, recovered from orphaned branches and merged into `main` 2026-07-06) but never reached Pillar 2 — PR 02-00/02-01/02-03 were created as empty placeholder branches and never built. Separately, the user has confirmed Grok Build is a paid xAI product (not, as assumed when this cluster was designed, a free SuperGrok entitlement), and has suspended its use after incurring real cost across two workspaces. This amendment does not change the design; it adds the gate that was missing for when a non-Grok agent needs to carry this pillar forward.

**The gate:** PR 02-01 through PR 02-04 (payload format, receipt writer, orchestrator core phases 0-2, post-gates + Build Ingestion Manifest generator) require no Grok capability at all — the PR Plan closing note above already states "After PR 02-04 a usable native-only path exists." Only **PR 02-05** (the write/review delegation to Grok `/design`) is Grok-specific. This amendment gates PR 02-05 on agent capability rather than assuming it:

- **If the executing agent has native Grok `/design` tool-calling available** (and the user has authorized its use for this session): follow PR 02-05 exactly as specified — emit the pointer/payload, delegate the write/review loop, resume at native post-gates.
- **If not** (Claude Code, or any agent without that tool-calling — the default going forward per the user's decision to step back from Grok Build "for a while"): the design-orchestrator performs the write/review loop **itself**, in-process. Concretely: the orchestrating agent writes the DESIGN doc directly from the assembled payload (Phase 0-2 output), then obtains an independent critique pass before treating any section final — either a self-critique against the payload's [INTENT] and gaps (minimum bar), or, where the runtime supports spawning a separate reviewing agent (e.g. Claude Code's Agent/subagent tooling), a genuinely independent reviewer pass to 0 open issues, mirroring Grok's mandatory-reviewer discipline without requiring Grok itself. The native post-gates in PR 02-04 (focus re-verify, quality chain, DESIGN_RECEIPTS emission, Manifest injection) are unchanged and run identically on both paths — they were never Grok-dependent to begin with.

**Evidence this works:** none of the 9 real commits already landed for Pillars 1 and 5 used Grok delegation — every one was written directly by the executing agent. The native path is not hypothetical; it is the one that actually produced verified, tested work in this cluster so far (238/238 tests, 0 CRITICAL lint, post-recovery). Only the Grok-specific PR in each pillar (02-05 here; 03-05 in Pillar 3) ever depended on a tool this agent may not have.

**PR Plan status update:** PR 02-05 is now labeled **conditional (Grok-available path)**. The native alternative for that slot is not a new PR — it is PR 02-03/02-04's existing orchestrator continuing one phase further (in-process write + independent review) rather than handing off, with no payload/pointer emission and no `.grok/` reference. PR 02-06 through 02-08 (integration, harden, end-to-end) proceed unchanged under either path.

**Key Decision (append to §"Key Decisions"):** Building a working non-Grok path is smaller than it looks, because 8 of this pillar's 9 PRs never needed Grok in the first place, and the 9th's *purpose* (an independent review pass before trusting a draft) is achievable without Grok's specific tool-calling. Gating one PR on capability — rather than deleting or rewriting the Grok-delegation path — preserves the option to use Grok later, consistent with /nodelete.
