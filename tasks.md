...
## Phase 4 — Verification-Spine Re-Verification — **4.1 DONE, 4.2 NOT YET READY** (4.2-4.5 remain Claude judgment/build work, not Gemini's — see note below)
...
- [ ] 4.2 Build `/execute-build`'s engine per the corrected design in `docs/compression-staging/execute-build-honest-design.md` Section 3: (a) add a CLI entrypoint to `scripts/focus/phase_status.py` (or a thin sibling wrapper) exposing the existing `TasksMdReport` as JSON, matching the `scripts/focus/focus.py` argparse/`main()`/`__main__` convention; (b) build a new small module (proposed: `scripts/build/build_evidence.py`) with two functions: a completeness scanner (file list → TODO/FIXME/PLACEHOLDER/bare-`pass`/`NotImplementedError` match list) and a scope-diff checker (workspace root + declared file list → git-status set difference); (c) wire both into `/execute-build`'s Step 0b/0d/6 (phase status) and Step 5d/5f (completeness/scope) as verification rails, replacing the current "read and assess by eye" instructions with real script calls whose JSON output the model reasons over; (d) real tests under `scripts/tests/` for the new module; (e) `.md` hardened per the recipe's Step F; (f) lint CLEAN. **This is Claude's build, not Gemini's** — per the established pattern (5 done engines were all natively built, never delegated), engine design requires the same Honest-Design judgment as 4.1 threaded through implementation; Gemini's role stays bounded mechanical text edits (the 2.5a/2.7 pattern), not new Python engine authorship.
