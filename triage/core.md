---
description: The Triage Desk Vector - A meta-workflow that recommends the appropriate next workflow(s) from the suite without the user having to know they exist.
---
...
**Design rule**: User intent is NEVER required. If not provided, /triage operates on state alone and produces full value. If provided as a single sentence at invocation, it modifies priority. /triage NEVER asks for intent if not given.
...
Read the user's session intent statement. Store it as `<SESSION_INTENT>`. If not provided, store as "not provided."
...
| Trigger | Priority | Intent Modifier |
|---------|----------|-----------------|
| New workflow `.md` file created with blank or missing frontmatter | P1 | → P0 if intent is "use this workflow now" |
| Any workflow `.md` file (monolithic) exceeds 10,000 bytes | P1 | → P0 if file > 12,000 bytes (truncation occurring) |
| A workflow pointer's `view_file` path references a file that no longer exists | P0 | |
...
When activated:
1. Read the invocation for a session intent statement (optional — proceed without it if absent)
...
This workflow operates at the intake layer of the workspace:
  ...
  1. /sentinel or manual invocation → THIS WORKFLOW
