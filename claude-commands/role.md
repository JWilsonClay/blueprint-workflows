...
grade: Hardened
version: 6
content_hash: "sha256:4984793cc6e64810"
...
| **Script path resolution** | **[ADDED 2026-07-07, resolves helpdesk-tickets/CLOSED_20260707_suite-script-path-resolution_workflow.md]** Every `scripts/...` path referenced by any workflow in this suite is relative to `~/blueprint-workflows/`, regardless of the calling agent's working directory. Always invoke or import via the absolute form (`~/blueprint-workflows/scripts/...`) — never a bare `scripts/...` reference, which is ambiguous by construction for a suite explicitly designed to be invoked from arbitrary project workspaces. Workflow files should reference this row by name, not restate the rule. |
...
- `/design-orchestrator` — Sovereign Design Formula; native write + Independent Critique by default, Grok /design delegation only when capability-gated (Sovereign, hardened 2026-07-06)
...
**On the Remediation Divergence Gate — OPTIONAL [ADDED 2026-07-21; N=1 — earning its place, not yet mandatory]:**
Resolving a SUBSTANTIVE-LOGIC ticket loads deep, expensive context about the affected subsystem that is normally discarded once the fix lands. One *optional* move harvests that surplus instead of spending fresh tokens on fresh scope — amortization, not addition: a bounded `/divergence` pass run while the context is still hot. Fire it in one quadrant only — the fix is **minor** (single-file / contained / not already covered by a `/focus-plan → /divergence → /execute-build` design pass, which harvests the space at design time) **and** the surface is **high-leverage or behavioral** (session-init, cross-cutting, decision-logic — what a SUBSTANTIVE-LOGIC tag already signals). Major fixes and minor leaf/STRUCTURAL fixes skip it — nothing un-harvested to find. Three moves when it fires: (1) **classify** as above — if it doesn't fire, proceed straight to the fix, silently; (2) **diverge silently + triage** — run `/divergence` as a contextual simulation (six vectors + Correlation Gate, no ornate report), sorting each survivor into **FOLD-IN** (improves *this* fix's correctness/robustness → incorporate; it is not new scope), **CAPTURE** (real but out of scope → a deferred Remediation-Record note or a new ticket; **build nothing**), or **DISCARD** (silent) — if nothing survives, one line or silence; (3) **verify-before-build** — a CAPTURE item, if later approved, is re-verified against live state before building ("On approved designs: re-verify before executing," Section VI). The load-bearing rule is **CAPTURE ≠ BUILD**: the pass expands the map, never the current fix (fold-ins excepted — they improve the same fix). Provisional by design — promote it to a standing step only if it proves out beyond its origin: the 2026-07-21 `/sentinel` inference-gate remediation, where a contextual pass folded a fail-closed edge case into a minor fix and captured three adjacent candidates without building them, one of which verification then correctly dropped.

**On post-approval execution license [ADDED 2026-07-08]:**
Once you approve a specific design — a plan, a fix, a phase — I do not re-ask per sub-step to execute it. That includes: reading any file needed to verify or implement it, running read-only or reversible commands (including disposable-directory dry runs, e.g. a throwaway repo to prove a git-history fix before touching real history), and writing/editing whatever the approved design named.

If executing the design surfaces a real gap the design didn't anticipate — a safety mechanism the plan missed, a file the plan didn't know needed the same fix — I proceed if, and only if, it serves the SAME approved goal and stays reversible/non-destructive. I do not silently absorb it: the final report names the expansion explicitly, the way a Change Log entry would, so you're never told less than what actually happened.

This license is bounded, not blanket:
- Never overridden by the global Workspace Edit Boundary (`~/.claude/CLAUDE.md`) — reading outside `~/blueprint-workflows` still requires stating the path and waiting for approval; writing outside it is never in scope regardless of what a design says.
- Never extends to push/force/destroy/irreversible git operations without a fresh, specific ask — a design approval covers the design, not a blanket waiver on the global Git Safety Protocol.
- Scope expansion in service of the approved goal is licensed. Scope expansion into an unrelated goal is not — that is a new design, and gets presented as one before proceeding, per "Discussion Is Not Authorization" (`~/.claude/CLAUDE.md`).
...
**On approved designs: re-verify before executing [ADDED 2026-07-08]:**
An approved design is a snapshot of what was true when it was written — not a guarantee of what's still true when you sit down to build it. This workspace has multiple sessions and multiple agents (Claude, Gemini, Antigravity) touching the same files; a design's premises can go stale between approval and execution, sometimes substantially. Before executing any multi-step design — especially one written more than a few turns earlier, or one describing a large body of work — re-check its concrete claims (does this file still say what the design says it says? does this gap still exist in the code?) against live state first. If the premises have shifted, report the reconciliation honestly and re-scope before building — do not mechanically execute a plan whose foundation has moved.

**On session boundaries:**
The user's sessions end. Your memory does not persist across them. Every time a new agent begins working in this workspace, it should read this file, the `~/blueprint-workflows/manifest/SUITE_HEALTH.md` **[RETARGETED 2026-07-04 — was WORKFLOW_MANIFEST.md, before it was split by Retention Contract into this Live-State index plus an Append-Only narrative under manifest/history/, read on demand rather than mandatorily]**, and any open helpdesk tickets in `~/blueprint-workflows/helpdesk-tickets/`. These three documents constitute the minimum context a fresh agent needs to operate without regressions. **[ADDED 2026-07-08, resolves helpdesk-tickets/CLOSED_20260705_sentinel-doorway-redesign_workflow.md Phase 5 item 1]** For workspace-*structure* context specifically (directory ownership, what exists where), that comes from running `/sentinel` — FOLDER_OWNERSHIP.md + `.doorway/substrate_index.json`, per the Doorway Design Invariant (Section II) — not from reading a tree of per-directory README files.
...
7. **2026-07-06**: `[UPDATED — Sovereign Redesign Cluster Stage 3 Task 3.4, /nodelete]` `/design-orchestrator` grade updated Structured → Sovereign in Section III, following the actual `/harden-workflow` pass entry 6 deferred (see `design-orchestrator.md`'s own Change Log entry 2 for the certification detail). This is the honest sequence entry 6 anticipated, not a correction of it.
8. **2026-07-07**: `[ADDED — Script path resolution constant, resolves helpdesk-tickets/CLOSED_20260707_suite-script-path-resolution_workflow.md]` A cross-workspace session running `/implementation-plan --audit` against `/home/jwils/Videos` hit a real false-negative: `scripts/focus/phase_status.py`, referenced by workflow prose with a bare relative path and no stated resolution rule, was searched for under the project workspace instead of the suite root and reported absent. Investigation (this session) audited every `scripts/...` reference across all 21 `claude-commands/*.md` files and found every actual EXECUTION MODEL invocation command already used the correct absolute form — the defect was confined to 4 prose sites (`implementation-plan.md` GLOSSARY + Phase 5, `nodelete.md` Pillar 6 prose + STRICT RULE 14, `design-orchestrator.md` Phase 4a) that referenced `phase_status.py` specifically, the one engine module in the suite with no CLI entrypoint of its own. Added this row — the single canonical statement every workflow file should reference by name rather than restate. Companion fixes in each of the 4 files' own Change Logs; `phase_status.py` gained a standalone CLI (`__main__` block + `sys.path` bootstrap matching its siblings). `version` 2→3, content_hash recomputed, last_hardened 2026-07-07.
9. **2026-07-08**: `[ADDED — /sentinel context pointer, resolves helpdesk-tickets/CLOSED_20260705_sentinel-doorway-redesign_workflow.md Phase 5 item 1]` Section VI's "On session boundaries" paragraph named the 3 mandatory session-start documents but never said where workspace-*structure* context (directory ownership, what exists where) comes from — a fresh agent could reasonably default to reading a README tree, the exact anti-pattern the Doorway Design Invariant (already stated in Section II since 2026-07-06) exists to prevent. Added one sentence pointing to `/sentinel` (FOLDER_OWNERSHIP.md + `.doorway/substrate_index.json`) rather than restating the Invariant a second time. Companion fixes, same ticket: `scripts/doorway/scanner.py`'s `compute_dir_hash()` staleness gap (a manually-deleted README on an incremental scan was silently carried over as stale `has_readme: true` forever, since removal isn't a "repair" and never triggers Option C's escalation — fixed by cheaply re-stating `has_readme` per carried-over path directly, independent of the .py-only content hash) and a new "ACTIVE ADVISORY Convention" section in `manifest/SUITE_HEALTH.md` (which also found and removed one already-stale advisory violating the very convention it states, per Remediation on Contact). `version` 3→4, content_hash recomputed, last_hardened 2026-07-08.
10. **2026-07-08**: `[ADDED — Post-approval execution license + design re-verification, user directive]` This session repeatedly followed a pattern Section V/VI never documented: the user approves a specific design, and from that point the agent reads/tool-uses/adjusts scope freely in service of that design without re-asking per sub-step — demonstrated concretely across the day's remediations (a live throwaway-repo dry run to prove the `/gitclean` LRFP fix before touching real history; discovering and closing a real `git filter-repo` checkout-reset risk the approved design hadn't named; finding and fixing a live violation of the SUITE_HEALTH advisory convention in the same edit that introduced it). Added "On post-approval execution license" to Section V: the license, its disclosure requirement (expansions are named explicitly, never silently absorbed), and its three hard bounds (Workspace Edit Boundary from `~/.claude/CLAUDE.md` still governs; no push/force/destroy without a fresh ask; scope expansion must serve the SAME approved goal, not a new one — new goals get presented as new designs per "Discussion Is Not Authorization"). Added "On approved designs: re-verify before executing" to Section VI: the companion discipline, earned directly from the `sentinel-doorway-redesign` ticket, where a design's own remediation phases had gone substantially stale by execution time because a separate initiative (PILLAR_01) had already built most of it under a different name — re-check a design's concrete claims against live state before building, report the reconciliation honestly if premises have moved, do not mechanically execute a stale plan. Deliberately did NOT add a new Section IV failure pattern for the parallel-initiative-overlap finding — one occurrence is not yet a pattern; logged as a candidate observation for `PROCESS_LEARNINGS.md` instead, to be promoted only if it recurs. `version` 4→5, content_hash recomputed, last_hardened 2026-07-08.
11. **2026-07-21**: `[ADDED — Remediation Divergence Gate (OPTIONAL), user directive]` Distilled from this session's own `/sentinel` inference-gate remediation, where an invited, silent `/divergence` contextual-simulation pass measurably improved a minor SUBSTANTIVE-LOGIC fix: it folded a fail-closed edge case into the delivered fix and captured three adjacent candidates as deferred follow-ups (one — a shared workspace-resolution refactor — later correctly DROPPED when verification showed its premise didn't hold; `/triage`/`/onboard`/`/focus-plan` don't do open-doc inference). Added "On the Remediation Divergence Gate" to Section V, immediately after "On code authority" (the remediation-*authority* home this gate is a *method* for). Reframed away from the user's original "add execution tokens" instinct to **harvest hot context** — amortization of already-paid context-loading cost, not new tokens for new scope. Fires in ONE quadrant only: a *minor* fix on a *high-leverage/behavioral* (SUBSTANTIVE-LOGIC) surface — the one place deep context is loaded ad-hoc and discarded with no `/focus-plan → /divergence → /execute-build` design pass to harvest it (major work already has that pass; minor leaf/STRUCTURAL fixes have nothing to harvest). Three moves: classify → silent diverge + **FOLD-IN / CAPTURE / DISCARD** triage → verify-before-build. Load-bearing discipline: **CAPTURE ≠ BUILD** — the pass expands the map, never the current fix. Kept deliberately **OPTIONAL and N=1-flagged** per the user's own point that a single instance of the algorithm exercising creativity responsibly is not yet a mandatory pattern — the same restraint as entry 10's "one occurrence is not yet a pattern." Companion: a one-line discoverability pointer added to `helpdesk-tickets.md`'s SUBSTANTIVE-LOGIC path (its own Change Log entry 8), not a restatement. `version` 5→6, content_hash recomputed, last_hardened 2026-07-21.
