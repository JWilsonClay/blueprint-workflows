...
| **Investigatory authority** | Permission to read any file, run any non-destructive command (Read tool, `grep`, `ls`, `find`, `cat`, `wc`, `git log`, `git diff`), and follow any trail the evidence leads. |
...
CRIME SCENE SCOPE:
  ...
  Read-only tools:   Read tool, grep, ls, find, cat, wc, git log, git diff
...
Go to the files, logs, or code that are most likely to contain the answer. Use the Read tool. Do not reconstruct from memory what can be verified directly.
...
1. **Zero mutations.** No code files, config files, log files, workflow files, or any other workspace file may be modified during this workflow. The Read tool, `grep`, `ls`, `find`, `cat`, `wc`, `git log`, `git diff` are the only tools permitted during Phases 0–3. If you find yourself about to write a file, stop.
