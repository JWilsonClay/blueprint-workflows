...
grade: Sovereign
version: 3
content_hash: "sha256:1b81c90a0c01b023"
...
5. **2026-07-06**: `[FIXED — receipt heredoc evaluation, Sovereign Redesign Cluster Stage 2, /nodelete]` The `HARDEN_GRADES.md` writer used a quoted heredoc delimiter (`<< 'RECEIPT_EOF'`), suppressing all `$()` command substitution — `$(date +%Y-%m-%d)` and the `$(git -C ... rev-parse ...)` commit line were never evaluated, writing literal shell syntax into the receipt instead of real values. Found live while exercising the identical pattern in `triage.md` this session; `HARDEN_GRADES.md`'s own existing entries were checked and do not carry this defect (prior agents evidently pre-substituted real values by hand before writing, rather than relying on the documented live-evaluation mechanism). Fixed by unquoting the delimiter; confirmed no backticks in the receipt body. Same fix applied to `iterate-test.md`, `document.md`, `soc.md`, `triage.md`, `execute-build.md` — see their own Change Logs.
