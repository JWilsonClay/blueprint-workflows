...
grade: Sovereign
version: 8
content_hash: "sha256:1434675f20150ede"
...
- **For a `tasks.md` phase specifically:** before archiving, consult `python3 ~/blueprint-workflows/scripts/focus/phase_status.py --workspace <workspace root> --output-json`'s receipt-cross-referenced status (it derives this from `.workflow_state/receipts/BUILD_RECEIPTS.md`, matching phase names exactly as `/execute-build` wrote them — see that script's own module docstring; path is absolute per role.md's Script path resolution constant — **[PATH-ANCHORED 2026-07-07, resolves helpdesk-tickets/CLOSED_20260707_suite-script-path-resolution_workflow.md]**). A phase whose derived status is one of `_COMPLETE_STATUSES` (`"phase complete"`, `"project build complete"`) or the legacy approx-match equivalent `found_complete_approx` may be archived. A phase reporting anything else — `not_started`, no matching receipt, or `tasks.md`/receipts absent entirely — is **not** archived; fire the Intent-Mismatch Gate (Pillar 4) instead: restate what you understood, surface the mismatch between "the user said archive this" and "the receipt trail doesn't confirm it's done," propose *not* archiving as the minimal repair, and ask. **[NOTE — 2026-07-07, Sovereign Redesign Cluster Pillar 4]** `/implementation-plan --audit`'s Completion Marking sub-pass (that workflow's Phase 5) runs the identical `phase_status.py` dual cross-reference *before* this gate ever sees the surface, and — only on independent double-confirmation — injects a human-visible `**COMPLETED [ARCHIVE:DATE]**` marker at the unit's own header. This is a complementary, advance-notice layer, not a replacement for this gate: this verification still runs at archive time regardless of whether a marker is present, and a marked unit whose receipt has since gone stale is still refused, not archived on the marker's word alone.
  **[DUAL-SURFACE ARCHIVAL — INJECTED 2026-07-08, Fix 3]** If the verification gate clears for a phase in `tasks.md`, and the workspace contains a corresponding planning document (e.g. `implementation-plan.md` or `implementation_plan.md`), the phase MUST be archived from **both surfaces concurrently**. The archiving agent must verify the matching phase header exists in the planning document; if it is absent or its name diverges, HALT and fire the Intent-Mismatch Gate.
...
**The action, once the gate clears:** move the named, verified-complete unit's content **verbatim** into `.history/archive/<filename>.ledger.md` as a new dated entry, then strip it from the active surface. In Dual-Surface Archival, this means appending a verbatim copy of the phase section from `tasks.md` to `.history/archive/tasks.md.ledger.md` and the corresponding phase section from `implementation-plan.md` to `.history/archive/implementation-plan.md.ledger.md`, before removing both sections from their respective active files. Everything not named — adjacent phases, unrelated sections, forward-looking work — is frozen exactly per the Safety Rail (Pillar 1). Archiving three phases when the user named three phases is N=N (the Spine); archiving a fourth because it also looked done is a scope violation.
...
14. **[ADDED 2026-07-04, resolves helpdesk-tickets/CLOSED_20260704_nodelete_workflow.md; UPDATED 2026-07-08 — Fix 3]** Archival Mode (Pillar 6) is strictly user-invoked (`/nodelete --archive` or its unambiguous prose equivalent) — never entered because a surface merely looks complete. For a `tasks.md` phase, verify completion via `~/blueprint-workflows/scripts/focus/phase_status.py`'s receipt-cross-referenced status (absolute path per role.md's Script path resolution constant — **[PATH-ANCHORED 2026-07-07, resolves helpdesk-tickets/CLOSED_20260707_suite-script-path-resolution_workflow.md]**) before archiving; any status other than the verified complete statuses (including the approx-match `found_complete_approx`) fires the Intent-Mismatch Gate instead of proceeding. **Dual-Surface Archival is mandatory**: once verified, a phase MUST be archived from both `tasks.md` and the corresponding planning document (e.g. `implementation-plan.md`) concurrently. Verify the phase header exists in the plan before archiving; if missing or mismatched, HALT and fire the Intent-Mismatch Gate. Archived content moves verbatim to `.history/archive/<filename>.ledger.md` — an Append-Only Ledger, not ingestion-banned, distinct from the Quarantined Change Ledger now at `.history/quarantine/`. Never archive a unit the user did not name, per the same Safety Rail that governs every other pillar.
...
See `.changelogs/nodelete.md` for the full history (10 entries, latest: 2026-07-08).
