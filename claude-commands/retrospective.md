...
| Term | Definition |
|------|------------|
| **PROCESS_LEARNINGS.md** | Global, cross-project institutional memory at `~/blueprint-workflows/process_learnings/PROCESS_LEARNINGS.md`. Append-only. Each /retrospective adds one entry. |
...
```bash
ls ~/blueprint-workflows/process_learnings/PROCESS_LEARNINGS.md || echo "MISSING"
```
...
```bash
mkdir -p ~/blueprint-workflows/process_learnings
cat <<EOF > ~/blueprint-workflows/process_learnings/PROCESS_LEARNINGS.md
# PROCESS_LEARNINGS.md — Global Workflow Institutional Memory
# Location: ~/blueprint-workflows/process_learnings/PROCESS_LEARNINGS.md
...
**5c. Atomic Append.**
Use the Bash tool with `cat >>` to ensure the new entry is appended to the substrate without reading/rewriting the entire file. This prevents truncation if the agent's context is overloaded.
...
**5d. Verification.**
```bash
tail -n 25 ~/blueprint-workflows/process_learnings/PROCESS_LEARNINGS.md
```
...
9. **[INJECTION 2026-05-08 — append safety]** Use shell-level redirection (`cat >>`) via the Bash tool for all appends to `PROCESS_LEARNINGS.md`. Never overwrite this file, as it risks silent truncation if the agent's context read of the existing file is partial or failed. Mechanical append is the only sovereign-grade method for ledgers.
...
Output: `~/blueprint-workflows/process_learnings/PROCESS_LEARNINGS.md`
...
3. **2026-05-21**: `[PORTED — Claude Code migration]` Pointer/Payload architecture retired. Merged into single command file at `~/blueprint-workflows/claude-commands/retrospective.md`. All Antigravity paths updated to `~/blueprint-workflows/process_learnings/PROCESS_LEARNINGS.md`. Phase 5 references to `run_command` tool updated to Bash tool. GLOSSARY PROCESS_LEARNINGS.md location updated.
