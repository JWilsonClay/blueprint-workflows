...
| **Hallucinated Success** | A Step 5 self-critique that finds zero weaknesses. The payload states explicitly: "a critique that finds nothing is almost certainly incomplete." If Step 5 produces no findings, that is itself a finding — the critique failed. Re-execute Step 5 with full rigor. |
...
Typical activation pattern:
  - User uploads /quality/core.md at session start
  - Agent reads and activates silently
